...
// htmlMaxBodyBytes limits how much of a response body is read (set from SEO_HTML_MAX_BODY_BYTES).
var htmlMaxBodyBytes int64 = 10 * 1024 * 1024

// SetHTMLMaxBodyBytes configures the body read limit (called at startup from env).
func SetHTMLMaxBodyBytes(n int64) {
	if n > 0 {
		htmlMaxBodyBytes = n
	}
}
...
func NewFromHTTPResponse(r *http.Response) (*models.PageReport, *html.Node, error) {
	...
	bodyReader = io.LimitReader(bodyReader, htmlMaxBodyBytes)
