...
type Options struct {
	...
	// RandomDelayMaxMs: random sleep 0..max ms before each request (0 = no delay).
	RandomDelayMaxMs int
	// CrawlerTimeoutHours: max duration for the whole crawl.
	CrawlerTimeoutHours int
	// ConsumerThreads: parallel fetch workers (minimum 1).
	ConsumerThreads int
}
...
func NewCrawler(parsedURL *url.URL, options *Options, client Client) *Crawler {
	...
	timeoutH := options.CrawlerTimeoutHours
	if timeoutH < 1 {
		timeoutH = 2
	}
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(timeoutH)*time.Hour)
...
func (c *Crawler) crawl() <-chan *ResponseMessage {
	...
	nThreads := c.options.ConsumerThreads
	if nThreads < 1 {
		nThreads = 1
	}
...
func (c *Crawler) consumer(reqStream <-chan *RequestMessage, respStream chan<- *ResponseMessage) {
	for {
		...
		case requestMessage := <-reqStream:
			...
			if c.options.RandomDelayMaxMs > 0 {
				time.Sleep(time.Duration(rand.Intn(c.options.RandomDelayMaxMs+1)) * time.Millisecond)
			}
