...
// PageReportIssueRow is a lightweight row for API responses (issue → affected URLs).
type PageReportIssueRow struct {
	ID         int64  `json:"id"`
	URL        string `json:"url"`
...
// ListPageReportsForIssueType returns up to limit pages that have the given issue_types.type for this crawl.
func (ds *PageReportRepository) ListPageReportsForIssueType(crawlID int64, issueType string, limit int) ([]PageReportIssueRow, error) {
	if limit < 1 {
		limit = 100
	}
	if limit > 500 {
	...
	query := `
		...
		ORDER BY url ASC
		LIMIT ?`
	rows, err := ds.DB.Query(query, crawlID, issueType, crawlID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var out []PageReportIssueRow
	for rows.Next() {
		var r PageReportIssueRow
		if err := rows.Scan(&r.ID, &r.URL, &r.Title, &r.StatusCode); err != nil {
			return nil, err
		}
		out = append(out, r)
	}
...
// PageDetailRow is the full evidence payload for a single crawled page — returned by the
// GET /api/seo/crawl/{id}/page/{page_id} endpoint so clients can display the specific values
// (title text, TTFB, images without alt, etc.) that triggered each reported SEO issue.
type PageDetailRow struct {
	...
	BlockedByRobotstxt bool             `json:"blocked_by_robotstxt"`
	Images             []ImageDetail    `json:"images"`
	ImagesMissingAlt   int              `json:"images_missing_alt_count"`
	Hreflangs          []HreflangDetail `json:"hreflangs"`
...
type HreflangDetail struct {
	URL  string `json:"url"`
	Lang string `json:"lang"`
}

// GetPageDetail returns the full evidence payload for a single page, verifying it belongs
// to the specified crawl. Returns nil if not found.
func (ds *PageReportRepository) GetPageDetail(crawlID, pageID int64) (*PageDetailRow, error) {
	// COALESCE on the computed nofollow expression so it never returns NULL when robots IS NULL.
	query := `
		SELECT
			id, url, status_code,
			redirect_url, content_type, media_type, lang,
			title, description, robots, noindex,
			COALESCE((robots LIKE '%nofollow%' OR robots LIKE '%none%'), 0) AS nofollow,
			canonical, h1, h2,
			COALESCE(words, 0), COALESCE(size, 0), depth, ttfb,
			in_sitemap, robotstxt_blocked
		FROM pagereports
		WHERE id = ? AND crawl_id = ?`

	row := ds.DB.QueryRow(query, pageID, crawlID)
	d := &PageDetailRow{}

	// Use sql.Null* for every nullable TEXT / INTEGER column in the schema.
	var (
	...
	err := row.Scan(
		&d.ID, &d.URL, &d.StatusCode,
		&redirectURL, &contentType, &mediaType, &lang,
		&title, &description, &robots, &noindexInt, &nofollowInt,
		&canonical, &h1, &h2,
		&d.Words, &d.SizeBytes, &d.Depth, &d.TTFBMs,
		&inSitemapInt, &blockedInt,
	)
	if err != nil {
		return nil, err
	}
	...
	d.Robots = robots.String
	d.Canonical = canonical.String
	d.H1 = h1.String
	d.H2 = h2.String
	d.Noindex = noindexInt != 0
	d.Nofollow = nofollowInt != 0
	d.InSitemap = inSitemapInt != 0
	d.BlockedByRobotstxt = blockedInt != 0
	d.TitleLength = len([]rune(d.Title))
	d.DescriptionLength = len([]rune(d.Description))

	// Images — alt TEXT is nullable in the schema
	irows, err := ds.DB.Query(`SELECT url, alt FROM images WHERE pagereport_id = ?`, pageID)
	if err != nil {
		log.Println(err)
	} else {
		defer irows.Close()
		for irows.Next() {
			...
			if err := irows.Scan(&imgURL, &imgAlt); err == nil {
				img := ImageDetail{URL: imgURL, Alt: imgAlt.String}
	...
	if d.Images == nil {
		d.Images = []ImageDetail{}
	}

	// Hreflangs — to_lang TEXT is nullable in the schema
	hrows, err := ds.DB.Query(`SELECT to_url, to_lang FROM hreflangs WHERE pagereport_id = ?`, pageID)
	if err != nil {
		log.Println(err)
	} else {
		defer hrows.Close()
		for hrows.Next() {
			var hURL string
			var hLang sql.NullString
			if err := hrows.Scan(&hURL, &hLang); err == nil {
				d.Hreflangs = append(d.Hreflangs, HreflangDetail{URL: hURL, Lang: hLang.String})
			}
		}
	}
	if d.Hreflangs == nil {
		d.Hreflangs = []HreflangDetail{}
	}

	// Issues (error type strings for this page in this crawl)
	isrows, err := ds.DB.Query(`
		SELECT issue_types.type
		FROM issues
		INNER JOIN issue_types ON issue_types.id = issues.issue_type_id
		WHERE issues.pagereport_id = ? AND issues.crawl_id = ?
		ORDER BY issue_types.type ASC`, pageID, crawlID)
	if err != nil {
		log.Println(err)
	} else {
		defer isrows.Close()
		for isrows.Next() {
			var t string
			if err := isrows.Scan(&t); err == nil {
				d.Issues = append(d.Issues, t)
			}
		}
	}
	if d.Issues == nil {
