...
var (
	...
	// TeX format: File `figure.png' not found (backtick open, quote close)
	...
	for i, line := range lines {
		...
		if reUndefinedSeq.MatchString(line) {
			...
			pl.Errors = append(pl.Errors, LogError{
				Line:    lineNum,
				Message: "Unknown LaTeX command: " + cmd + " on line " + strconv.Itoa(lineNum),
				Raw:     line,
			})
			...
			pl.Errors = append(pl.Errors, LogError{
				Line:    lineNum,
				Message: "Math mode error near line " + strconv.Itoa(lineNum),
				Raw:     line,
			})
			...
			pl.Errors = append(pl.Errors, LogError{
				Line:    lineNum,
				Message: "Image or file not found: " + fn,
				Raw:     line,
			})
			...
			pl.Errors = append(pl.Errors, LogError{
				Line:    lineNum,
				Message: "Missing closing brace near line " + strconv.Itoa(lineNum),
				Raw:     line,
			})
			...
			pl.Errors = append(pl.Errors, LogError{
				Line:    lineNum,
				Message: "Fatal error — check syntax near line " + strconv.Itoa(lineNum),
				Raw:     line,
			})
		} else if reOverfullHbox.MatchString(line) {
			lineNum := extractLineNum(lines, i)
			pl.Warnings = append(pl.Warnings, LogWarning{
				Line: lineNum,
				Text: "Overfull \\hbox",
				Raw:  line,
			})
